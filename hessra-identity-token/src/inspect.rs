...
///
/// Uses `print_block_source()` to get correct check rule text for each block,
/// then parses for the pattern: `$a == "identity"`
/// This is generated by `add_identity_attenuation_to_token` and works for any
/// identity format, including domains with colons (e.g., `uri:urn:test:service`).
fn extract_delegated_identity(biscuit: &Biscuit) -> Option<String> {
    let block_count = biscuit.block_count();

    // Search blocks in reverse (last block = most specific delegation).
    for i in (1..block_count).rev() {
        let source = biscuit.print_block_source(i).unwrap_or_default();
        for line in source.lines() {
            let trimmed = line.trim();
            if let Some(pos) = trimmed.find("$a == \"") {
                let after_eq = &trimmed[pos + 7..];
                if let Some(end_quote) = after_eq.find('"') {
                    let identity = &after_eq[..end_quote];
...
mod tests {
    ...
    #[test]
    fn test_inspect_delegated_token_with_nonstandard_domain() {
        let keypair = KeyPair::new();
        let public_key = keypair.public();
        // Domain with colons that doesn't match urn:/https:/mailto: or contain "hessra"
        let base_identity = "uri:urn:test:argo-cli1".to_string();
        let delegated_identity =
            "uri:urn:test:argo-cli1:01ABCDEFGHIJKLMNOPQRSTUVWX:01ZYXWVUTSRQPONMLKJIHGFEDCB"
                .to_string();
        ...
        assert_eq!(
            result.identity, delegated_identity,
            "Should extract delegated identity even with non-standard domain format"
        );
        assert!(result.is_delegated);
    }
